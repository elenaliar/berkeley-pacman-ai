...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        "start state einai i thesi ekinisis tou pacman kai oi 4 unvisited gonies"
        self.startState = (self.startingPosition, self.corners)
        self.startingGameState = startingGameState#auto to xrisimopoio gia tin klisi tis mazedistance sto cornersHeuristic
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        "len(state[1]) = poses gonies  apomenoun na episkeftei"
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            for x in state[1]:
                if (x != next_state):
                    unvisited_corner.add(x)
            tup = tuple(unvisited_corner)
            action_cost = self.getActionCost(state, action, next_state)
            children.append(((next_state, tup), action, action_cost))
...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        ...
        unvisited_corner = list(state[1])
        distance = set()
    # ipologizoume tin apostasi distance apo to current position(state[0]) stin gonia x 
        for x in unvisited_corner:
            #distance.add(manhattanDistance(state[0], x))#doulevei kai auto pairnontas tous idious pontous me tin maze distance
            distance.add(mazeDistance(state[0], x, problem.startingGameState))
    # epistrefei  tin apostasi gia tin gonia pou einai makritera
        return max(distance)
...
def foodHeuristic(state, problem):
    ...
    distance = set()
    for x in foodlist:
        #distance.add(manhattanDistance(position, x))#stin arxi eixa lisei to provlima me to manhattan distance alla den pernage ola ta test cases giati ekane expand pollous komvous
        distance.add(mazeDistance(position, x, problem.startingGameState))#me to mazedistance argei ligo alla pernaei ta testcases
    #epestrepse tin apostasi tou fagitou pou brisketai pio makria
    return max(distance)
